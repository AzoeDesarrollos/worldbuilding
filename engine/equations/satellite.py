...
class Satellite:
    ...
    def calculate_density(ice, silicate, iron):
        comp = {'water ice': ice / d(100), 'silicates': silicate / d(100), 'iron': iron / d(100)}
        density = q(sum([comp[material] * d(material_densities[material]) for material in comp]), 'g/cm^3')
        return density
...
class Major(Satellite, BodyInHydrostaticEquilibrium):
    ...
    def __init__(self, data):
        ...
        self.mass = q((d(self.radius.m) ** 3 * self.density.to('earth_density').m), 'earth_mass')
        self.gravity = q(self.mass.m / (d(self.radius.m) ** 2), 'earth_gravity')
...
class Minor(Satellite):
    ...
    def __init__(self, data):
        ...
        self.volume = q(d(4 / 3) * pi * _a * _b * _c, 'km^3')
...
class RockyMoon(Satellite):
    ...
    def set_density(self, composition: dict):
        ...
        s = composition['silicates'] if 'silicates' in composition else roll(0.6, 0.9) if percent < 100 else 0
        i = composition['water ice'] if 'water ice' in composition else roll(1 - s, 0.9 - s) if percent < 100 else 0
        r = composition['iron'] if 'iron' in composition else 1 - (s + i) if percent < 100 else 0
        return self.calculate_density(d(i), d(s), d(r))
...
def _moon_composition(data):
    ice = d(data['composition'].get('water ice', 0))
    rock = d(data['composition'].get('silicates', 0))
    iron = d(data['composition'].get('iron', 0))
