...
def render_textrect(string, font, width, text_color, background_color, justification=0):
        ...
        if font.size(requested_line)[0] > width:
            ...
            for word in words:
                ...
                if font.size(test_line)[0] < width:
                    ...
                    h += font.size(accumulated_line)[1]
                    final_lines.append(accumulated_line)
                    accumulated_line = word + " "
            h += font.size(requested_line)[1]
            final_lines.append(accumulated_line)
        else:
            h += font.size(requested_line)[1]
    ...
    for line in final_lines:
        if accumulated_height + font.size(line)[1] > h:
